...
async function getItem(id: string) { const r = await api.get(`/inventory/items/${id}`); return r.data; }
...
export default function ItemDetailPage() {
  ...
  const movements: any[] = Array.isArray(item?.movements) ? item.movements : [];
    ...
    <div dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
          ...
          <p className={`text-4xl font-bold ${isLow ? 'text-red-600' : 'text-gray-900'}`}>{item.currentStock}</p>
          <p className="text-xs text-gray-400 mt-1">{UNIT_LABEL[item.unit] ?? item.unit}</p>
          <p className="text-xs text-gray-500 mt-2">מינ': {item.minStockLevel}</p>
          <p className="text-xs text-gray-500">ערך מלאי: {fmtCurrency(item.currentStock * item.costPrice)}</p>
        ...
        <div className="md:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-5">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
            {[
              ...
              ['יחידה', UNIT_LABEL[item.unit] ?? item.unit],
              ['מע"מ', `${(item.vatRate * 100).toFixed(0)}%`],
              ['מחיר קנייה', fmtCurrency(item.costPrice)],
              ['מחיר מכירה', fmtCurrency(item.sellingPrice)],
            ].map(([k, v]) => (
              <div key={k}>
