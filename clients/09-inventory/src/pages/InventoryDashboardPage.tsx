...
async function getItems()     { const r = await api.get('/inventory/items', { params: { pageSize: 100 } }); return r.data; }
...
export default function InventoryDashboardPage() {
  ...
  const lowStock     = items.filter(i => i.currentStock <= i.minStockLevel && i.isActive);
  const totalValue   = items.reduce((s, i) => s + (i.currentStock * (i.costPrice ?? 0)), 0);
            ...
            <tbody className="divide-y divide-gray-100">
              {items.slice(0, 10).map((item: any) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5">
                    <span className={`font-medium ${item.currentStock <= item.minStockLevel ? 'text-red-600' : 'text-gray-900'}`}>
                      {item.currentStock}
                    </span>
                  </td>
                  <td className="px-4 py-2.5 text-gray-500">{item.minStockLevel}</td>
                  <td className="px-4 py-2.5">{fmtCurrency(item.sellingPrice)}</td>
                </tr>
              ))}
            </tbody>
          </table>
