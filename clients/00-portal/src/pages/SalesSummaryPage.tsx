...
export default function SalesSummaryPage() {
  const now = new Date();
  const [period, setPeriod] = useState(now.toISOString().split('T')[0]);

  const { data, isLoading, refetch } = useQuery({
    queryKey: ['pos-daily', period],
  ...
  const KPI = [
    { label: 'סה"כ מכירות', value: fmtCurrency(summary.totalSales ?? summary.totalRevenue ?? 0), icon: TrendingUp, color: 'indigo' },
    { label: 'מספר עסקאות', value: summary.salesCount ?? summary.transactionCount ?? 0, icon: ShoppingBag, color: 'green' },
    { label: 'ממוצע לעסקה', value: fmtCurrency(summary.averageSale ?? summary.averageTransaction ?? 0), icon: CreditCard, color: 'blue' },
    { label: 'מזומן', value: fmtCurrency(summary.byPaymentMethod?.CASH ?? summary.cashTotal ?? 0), icon: Banknote, color: 'yellow' },
  ];
