...
async function getSales(params: Record<string, string>) {
  ...
  const r = await api.get(`/pos/sales${q ? '?' + q : ''}`);
  return r.data;
}
            ...
            <tbody className="divide-y divide-gray-100">
              {sales.map((s: any) => (
                <tr key={s.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-gray-600">{s.saleNumber ?? s.id?.slice(-8)}</td>
                  <td className="px-4 py-3 text-gray-700">
                    {Array.isArray(s.items)
                      ? s.items.map((i: any) => `${i.item?.name ?? i.itemId} ×${i.quantity}`).join(', ')
                      : `${s.itemCount ?? '—'} פריטים`}
                  </td>
