...
const NI_LOWER_THRESHOLD  = 7_700;  // ≈60% מהשכר הממוצע 2026 — עודכן 2026-01-01
...
const NI_EMP_BELOW    = 0.035;  // 3.50% עד תקרה נמוכה — עודכן 2026-01-01
const NI_EMP_ABOVE    = 0.12;   // 12.00% מעל תקרה נמוכה — עודכן 2026-01-01
...
const MINIMUM_WAGE_MONTHLY = 5_880;  // שכר מינימום חודשי 2026 — עודכן 2026-01-01 (לאמת בינואר/יולי)
const MINIMUM_WAGE_HOURLY  = 33.91;  // שכר מינימום שעתי 2026 (5,880 ÷ 173.4 שעות חודשיות)
...
// IMPORTANT: Verify annually at: https://www.gov.il/he/departments/guides/irgunit_vacation
// Last verified: 2026-01-01
const RECUPERATION_DAILY_RATE = 438; // ₪ ליום — צו הרחבה 2026
...
export function calcTrainingFund(
  ...
  // הנוסחה: (שכר מעל תקרה) × max(0, שיעור_מעסיק - 4.5%)
  const MAX_EXEMPT_ER_PCT = 4.5;
  const excessSalary = Math.max(0, salaryBase - TRAINING_FUND_SALARY_CEILING);
  const taxableExcess = excessSalary > 0
    ? round2(excessSalary * Math.max(0, erRate - MAX_EXEMPT_ER_PCT) / 100)
    : 0;
